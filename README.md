...
Когда агент работает как системный сервис, stdout никто не видит - поэтому при `run`
(это именно то, что реально исполняет сервис) агент сам пишет подробный структурированный
(JSON, по одной строке на событие) лог в файл, по одному файлу на день:
`<папка с agent.json>/logs/agent-ГГГГ-ММ-ДД.log`. Хранится 14 дней, старое чистится само.
Путь к текущему лог-файлу и всегда виден в шапке меню (запусти `pingachock-agent` ещё раз).

В логе - вообще всё: каждый опрос бекенда (успех/ошибка, сколько заданий), каждый HTTP-запрос
с таймингом на уровне транспорта (в т.ч. отдельно фиксируются TLS-хендшейк и ошибки dial
для резервного Cloud Run канала), каждая выполненная проверка (тип/таргет/латентность/ошибка),
переключение Direct → Fronted при отказе прямого канала.

Сводка на главном экране меню (без необходимости лезть в лог) - статус сервиса, какой
транспорт сейчас активен, когда был последний опрос и чем он закончился, количество ошибок
подряд, когда последний раз успешно отправлялись результаты, с какого момента сервис работает.
