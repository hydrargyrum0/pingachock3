...
- Всё это превращается в `checks.NetConfig{LocalAddr, Resolver}`, который прокидывается в
  ...
  интерфейса.
- Транспорт узел↔бекенд (`internal/transport`) **не** биндится на выбранный интерфейс —
  он всегда идёт через системный маршрут по умолчанию (`program.Start`/`run` в
  `cmd/agent/main.go` передаёт `nil` вместо `LocalAddr` в `transport.NewDirect`/`NewFronted`).
  Раньше биндился так же, как и проверки, но это ловило реальные обрывы в проде: путь,
  привязанный к конкретному физическому интерфейсу, может быть недоступен (заблокирован
  маршрут, особенность ISP) даже когда путь по умолчанию работает нормально — живой пример
  из Туркменистана на Windows: `dial tcp 192.168.1.8:0->142.251.24.100:443: ... unreachable
  network` при биндинге, при этом системный маршрут по умолчанию до того же адреса рабочий.
  Выбор интерфейса при `configure` влияет только на то, откуда идут сами проверки
  (`checks.NetConfig`), не на связь узла с бекендом.
- Если оператор не запускал `configure` (или интерфейс определить не удалось) — `LocalAddr`/
  `Resolver` остаются `nil`, и поведение проверок не отличается от системного умолчания
  (fail-safe, не ломает работу агента, просто не даёт гарантии "видим ровно то, что видит
  ISP").
...
2. **Проверка бекенда и выбор интерфейса — два разных, независимых шага.** `resolveConfig`
   сначала проверяет сам бекенд один раз через `probeBackend` — без привязки к интерфейсу,
   тем же путём (системный маршрут по умолчанию), которым реально будет ходить работающий
   агент: сначала direct, если не вышло и настроен резервный канал, то fronted. Это ловит
   опечатки в `node_secret`/URL/резервном канале независимо от того, какой интерфейс потом
   выберут для проверок. Отдельно `chooseInterface` перебирает интерфейсы и информационно
   (не блокируя настройку) стучится с каждого через `probeInterface` — это влияет только на
   то, откуда пойдут сами проверки (ping/tcp/http/dns по целям), а не на связь с бекендом.
   Раньше оба шага были одним: транспорт узел↔бекенд биндился на выбранный интерфейс, и если
   бинд к конкретному физическому интерфейсу не мог достучаться (активный VPN/relay
   перехватывает маршрут по умолчанию, или путь до конкретного адреса заблокирован для
   привязанного источника — конкретный случай из Туркменистана на Windows: `dial tcp
   192.168.1.8:0->...:443: ... unreachable network`, при рабочем маршруте по умолчанию),
   агент не мог связаться с бекендом вообще, хотя реальной причины для этого не было.

Обе проверки идут через настоящий протокол (`Poll` с `node_secret`), не через `/healthz` —
важно, потому что fronting-прокси (`deploy/fronting-proxy`) намеренно проксирует только
