...
internal/agentlog/      — структурированный (JSON) лог в файл с ежедневной ротацией -
                           при работе как сервис stdout никто не видит, это единственный
                           способ понять постфактум, что пошло не так
internal/agentstate/    — снимок "что сейчас делает агент" (последний опрос, транспорт,
                           ошибки подряд) в JSON-файл, читается меню отдельного запуска
                           бинарника - процессы разные, напрямую не общаются
```
...
### Автодиагностика при установке

Живая отладка (см. историю в репо/чате) вскрыла два класса проблем, которые узел не мог
обнаружить сам и молча оставался "настроен, но не работает". Обе теперь исправляются
автоматически при `setup`/`configure`, без ручного вмешательства:

1. **"Опасные" директории.** Установка сервиса в `~/Documents`/`~/Desktop`/`~/Downloads` на
   macOS ловит TCC (даже root не может прочитать оттуда файл без Full Disk Access), а на
   Windows — Defender Controlled Folder Access (то же самое для процесса, запущенного как
   SYSTEM). Перед `install` агент сам определяет риск (`riskyDir`), копирует себя в
   `/usr/local/pingachock-agent` (Unix) или `%ProgramData%\pingachock-agent` (Windows) и
   перезапускается оттуда (`relocateIfRisky` + `reExecFrom`). Если перенести не вышло (нет
   прав) — не падает, просто продолжает с исходного места.

2. **Интерфейс, который "up", но не может достучаться до бекенда.** Частый случай: активный
   VPN/relay (iCloud Private Relay, корпоративный VPN, свой же VPN-продукт) перехватывает
   маршрут по умолчанию, а сырой путь с конкретного физического интерфейса до бекенда
   зависает/недоступен. Раньше это узнавалось только руками (30-секундные зависания в логе).
   Теперь `chooseInterface` перед сохранением конфига реально стучится на `/api/v1/agent/poll`
   через каждый кандидат (`probeInterface`, таймаут 8с — не 30, кто-то ждёт у экрана) —
   сначала direct, если не вышло и настроен резервный канал, то fronted. При `-interface=auto`
   выбирается первый рабочий; при интерактивном выборе список показывается с пометками
   ("OK"/"недоступен: <причина>"), дефолт — первый рабочий. При явном `-interface=NAME`
   (скриптовый/невнимательный запуск) — предупреждает, но не блокирует, раз это может быть
   временный сбой.

Проверка идёт через настоящий протокол (`Poll` с `node_secret`), не через `/healthz` — важно,
потому что fronting-прокси (`deploy/fronting-proxy`) намеренно проксирует только
`/api/v1/agent/*`, `/healthz` он либо не форвардит, либо отвечает сам за себя. Проверка через
`/healthz` дала бы ложное "недоступен" для рабочего fronted-канала.

launchd на macOS отдельно ловил флейковость: `kardianos`-совский `start` идёт через устаревший
`launchctl load` и после нескольких install/uninstall может падать с неинформативным
`Load failed: 5: Input/output error`. `startService` при такой ошибке сам гоняет
`launchctl bootout` + `launchctl bootstrap` напрямую (это же чинит проблему при ручной отладке)
