...
Осознанно не делаю сейчас self-serve `POST /api-keys` — ключи пока выдаются вручную (ты единственный оператор), добавится как endpoint, когда появится второй внешний потребитель.
...
Всего 9 эндпоинтов v1 — покрывает полный цикл (заявка → диспетчеризация → результат → доступ извне), без раздутия.
