...
func NewFronted(localAddr net.IP, frontDomain, realHost, nodeSecret string, timeout time.Duration) *HTTPTransport {
	...
	rt := &http.Transport{
		DialTLSContext: func(ctx context.Context, network, _ string) (net.Conn, error) {
			...
			conn, err := dialer.DialContext(ctx, network, net.JoinHostPort(frontDomain, "443"))
			if err != nil {
				return nil, err
			}
	...
	return NewHTTPTransport(client, "https://"+realHost, nodeSecret)
}
